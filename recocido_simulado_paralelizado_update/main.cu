...
double newSolution_v2(int n_students, int n_colegios, int totalVuln, int aluxcol[], int aluVulxCol[], int cupoArray[], double** distMat, int currentSolution[], const double alpha[]);
///////////////////////////////////////////////////
...
double min_temp = 0.00009;//0.00000009; // Minima temperatura que puede llegar
...
std::uniform_int_distribution<int> distBlocks(0, 0);
std::uniform_int_distribution<int> distThreads(0, 0);
...
int main(int argc, char *argv[]) {
    ...
    distBlocks = std::uniform_int_distribution<int>(0, n_block - 1);
    distThreads = std::uniform_int_distribution<int>(0, n_thread - 1);
    ...
    while(temp > min_temp){
        ...
        ///////////////////////////////////////////////////
        /* Siempre selecciona como nueva solución a la mejor que encontro entre las que genero*/
        /*
        ...
        */
        ...
        /* Si la mejor solución encontraada es peor que el actual, selecciona uno al azar*/
        /*
        * Alternativa A si es mayor
        */
        ...
        */
        /*
        * Alternativa B si es mayor y acepta por la función acepta(costPreviousSolution,costCurrentSolution)
        */

        
        if (costCurrentSolution > costPreviousSolution && acepta(costPreviousSolution,costCurrentSolution)) {
        


            // Alternativa A
            // Selecciono a uno aleatorio dentro de la totalidad de los colegios y estudiantes del universo de soluciones vecinas.
            ...
            */
            /// Alternativa B
            // Selecciono a uno aleatorio dentro de la totalidad de los colegios explorados y los estudiantes explorados.
            ///

            /*
            selectSch = distThreads(mt);
            selectStu = distBlocks(mt);
            */

            /// Alternativa C
            // Selecciono al primer estudiante y escuela de la lista shuffle.
            ///
            
            selectSch = 0;
            selectStu = 0;

            aluxcol[currentSolution[shuffle_student[selectStu]]] -= 1; ///
            aluVulxCol[currentSolution[shuffle_student[selectStu]]] -= alumnosSep[shuffle_student[selectStu]]; ///
            aluxcol[shuffle_colegios[selectSch]] += 1; ///
            aluVulxCol[shuffle_colegios[selectSch]] += alumnosSep[shuffle_student[selectStu]]; ///
            currentSolution[shuffle_student[selectStu]] = shuffle_colegios[selectSch]; ///
            costCurrentSolution = newSolution_v2(n_students, n_colegios, totalVuln, aluxcol, aluVulxCol, cupoArray, distMat, currentSolution, alpha);
        }
        else{
            //vector_historyAsign.push_back(currentSolution[shuffle_student[selectBlock]]);
            aluxcol[currentSolution[shuffle_student[selectBlock]]] -= 1; ///
        ...
        if(costCurrentSolution < costBestSolution){
            ...
            count_rechaso=0;
            std::cout << costCurrentSolution << std::endl;
        }
        else{
