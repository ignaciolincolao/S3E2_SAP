...
void shuffle_matrix(int[], const int max_change, const int max_col, std::uniform_int_distribution<int> distri);
...
__global__ void newSolution_kernel(
    ...
    colchange = d_shuffle_colegios[blockIdx.x * pitch_shuffle / sizeof(int) + threadIdx.x];
    ...
    while(salto){
        offset = salto-(myID+1);
        if(offset>myID){
                if(solutions[myID]>solutions[offset]){
                    solutions[myID]=solutions[offset];
                    solutions_thread[myID]=solutions_thread[offset];
                }
        }
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "Max promedio dist:" << max_dist << "\n";
    ///////////////////////////////////////////////////
    /// Registro de datos
    ///////////////////////////////////////////////////
    ...
    int *shuffle_colegios = (int *) malloc(sizeof(int) * n_block * n_colegios);
    for (int i = 0; i < n_students; i++) {
        shuffle_student[i] = i;
    }
    for (x=0; x<n_block; x++){
        for(z=0; z<n_colegios; z++){
            shuffle_colegios[n_colegios * x + z]=z;
        }
    }
    ...
    size_t pitch_shuffle;
    cudaMallocPitch(&d_shuffle_colegios,
                    &pitch_shuffle,
                    n_colegios * sizeof(int),
                    n_block); // Reserva memoria para la matriz de distancia

        
    size_t h_pitchBytes_shuffle = n_colegios * sizeof(int);
    size_t h_pitchBytes = n_colegios * sizeof(double);
    cudaMemcpy2D(d_shuffle_colegios,
        pitch_shuffle,
        shuffle_colegios,
        h_pitchBytes_shuffle,
        n_colegios * sizeof(int),
        n_block,
    ...
    while(temp > min_temp){
        ...
        shuffle_matrix(shuffle_colegios,n_thread,n_block,dist2);
        ...
        ///////////////////////////////////////////////////
        ///  Envia datos matriz de colegios
        ///////////////////////////////////////////////////
        ...
        //std::cout << "resultado final: " << selectBlock << " | " << selectThread << "\n";
        
        //if(currentSolution[shuffle_student[selectBlock]] == selectThread){
        //    std::cout << "viejo colegio: " << currentSolution[shuffle_student[selectBlock]] << " | Nuevo colegio: " << selectThread<< "\n";
        //}
        ...
        ///////////////////////////////////////////////////
        //std::cout << "Costo porr GPU" << costCurrentSolution << "\n";
        //std::cout << "Costo en CPU" << calCosto(currentSolution) << "\n";
...
void shuffle_matrix(int values[], const int max_change, const int max_row, std::uniform_int_distribution<int> distri) {
    int randvalue1,randvalue2,tem_value;
    for (int x = 0; x<max_row; x++){
        for (int z = 0; z<max_change; z++) {
            randvalue1 = distri(mt);
            randvalue2 = z;
            tem_value = values[n_colegios * x + randvalue1];
            values[n_colegios * x + randvalue1] = values[n_colegios * x + randvalue2];
            values[n_colegios * x + randvalue2] = tem_value;
        }
    }

}

/* Los deja como si fuerran todos iguales 
void shuffle_matrix(int values[], const int max_change, const int max_row, std::uniform_int_distribution<int> distri) {
    int randvalue1,randvalue2,tem_value;
    for (int z = 0; z<1; z++) {
        randvalue1 = distri(mt);
        randvalue2 = z;
        tem_value = values[n_colegios * 0 + randvalue1];
        values[n_colegios * 0 + randvalue1] = values[n_colegios * 0 + randvalue2];
        values[n_colegios * 0 + randvalue2] = tem_value;
    }
    for (int x = 0; x<max_row; x++){
        for (int z = 0; z<max_change; z++) {
            values[n_colegios * x + z] = values[n_colegios * 0 + z];
        }
    }
